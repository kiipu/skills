...
The published `kiipu-cli` skill covers the full Kiipu CLI command surface used from Claude Code:

- `kiipu note`
  list, search, show, update, star, pin, create, delete, restore, and purge
- `kiipu auth`
  login, `--device-name`, `--no-browser`, `--api-key`, status, and logout
- `kiipu doctor`
- `kiipu skills`
- `kiipu help`
  root help plus `note`, `auth`, `doctor`, and `skills`
